...
unsafe extern "C" fn url_constructor(cx: *mut JSContext, argc: c_uint, vp: *mut JSVal) -> bool {
    ...
    rooted!(in(cx) let url_obj = JS_NewPlainObject(cx));
    if url_obj.get().is_null() {
        args.rval().set(UndefinedValue());
        return false;
    }

    if define_function(cx, url_obj.get(), "toString", Some(url_to_string), 0).is_err() {
        args.rval().set(UndefinedValue());
        return false;
    }
    if define_function(cx, url_obj.get(), "toJSON", Some(url_to_json), 0).is_err() {
        args.rval().set(UndefinedValue());
        return false;
    }

    if set_url_properties(cx, url_obj.get(), &resolved).is_err() {
        args.rval().set(UndefinedValue());
        return false;
    }
...
unsafe extern "C" fn url_to_string(cx: *mut JSContext, _argc: c_uint, vp: *mut JSVal) -> bool {
    let args = CallArgs::from_vp(vp, 0);
    if let Some(href) = get_this_href(cx, &args) {
        args.rval().set(create_js_string(cx, &href));
        true
    } else {
        args.rval().set(UndefinedValue());
        false
...
unsafe extern "C" fn url_search_params_constructor(
    ...
    let raw = if argc > 0 {
        params_raw_from_value(cx, *args.get(0))
    } else {
        String::new()
    };

    match create_url_search_params_object(cx, &raw) {
        Ok(obj) => {
...
unsafe extern "C" fn url_search_params_append(
    ...
    let args = CallArgs::from_vp(vp, argc);
    let name = if argc > 0 {
        js_value_to_string(cx, *args.get(0))
    } else {
        String::new()
    };
    let value = if argc > 1 {
        js_value_to_string(cx, *args.get(1))
    } else {
        String::new()
    };

    let mut pairs = params_pairs_from_this(cx, &args);
    pairs.push((name, value));
    let _ = write_params_pairs_to_this(cx, &args, &pairs);

    args.rval().set(UndefinedValue());
    true
}

unsafe extern "C" fn url_search_params_set(cx: *mut JSContext, argc: c_uint, vp: *mut JSVal) -> bool {
    let args = CallArgs::from_vp(vp, argc);
    let name = if argc > 0 {
        js_value_to_string(cx, *args.get(0))
    } else {
...
unsafe extern "C" fn url_search_params_get_size(
    ...
    let args = CallArgs::from_vp(vp, argc);
    let size = params_pairs_from_this(cx, &args).len() as i32;
    args.rval().set(Int32Value(size));
    true
}

unsafe extern "C" fn url_search_params_get(cx: *mut JSContext, argc: c_uint, vp: *mut JSVal) -> bool {
    let args = CallArgs::from_vp(vp, argc);
    let name = if argc > 0 {
        js_value_to_string(cx, *args.get(0))
    } else {
    ...
    let value = params_pairs_from_this(cx, &args)
        .into_iter()
        .find(|(k, _)| *k == name)
        .map(|(_, v)| v);

    if let Some(v) = value {
        args.rval().set(create_js_string(cx, &v));
    } else {
...
unsafe extern "C" fn url_search_params_has(cx: *mut JSContext, argc: c_uint, vp: *mut JSVal) -> bool {
    let args = CallArgs::from_vp(vp, argc);
    let name = if argc > 0 {
        js_value_to_string(cx, *args.get(0))
    } else {
    ...
    let has = params_pairs_from_this(cx, &args)
        .into_iter()
        .any(|(k, _)| k == name);
...
unsafe extern "C" fn url_search_params_to_string(
    ...
    let args = CallArgs::from_vp(vp, 0);
    let encoded = serialize_params_pairs(&params_pairs_from_this(cx, &args));
    args.rval().set(create_js_string(cx, &encoded));
    true
}
...
unsafe fn set_url_properties(cx: *mut JSContext, obj: *mut JSObject, url: &Url) -> Result<(), String> {
    ...
    rooted!(in(cx) let params_val = ObjectValue(params_obj));
...
unsafe fn get_this_href(cx: *mut JSContext, args: &CallArgs) -> Option<String> {
    let this_val = args.thisv();
    if !this_val.get().is_object() || this_val.get().is_null() {
        return None;
    }

    rooted!(in(cx) let this_obj = this_val.get().to_object());
...
unsafe fn params_raw_from_value(cx: *mut JSContext, value: JSVal) -> String {
...
unsafe fn params_pairs_from_this(cx: *mut JSContext, args: &CallArgs) -> Vec<(String, String)> {
    let this_val = args.thisv();
    if !this_val.get().is_object() || this_val.get().is_null() {
        return Vec::new();
    }
