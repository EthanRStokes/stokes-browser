...
unsafe fn bridge_custom_prototype_to_wrapper_prototype(
    ...
    // Class prototypes are usually chained to HTMLElement.prototype in this runtime.
    // Re-point them to the wrapper prototype so upgraded instances keep DOM methods.
    set_object_prototype(cx, custom_proto, wrapper_proto);

    // Some pages hit environments where prototype re-linking is not reflected reliably.
    // Copy essential Element methods directly as a fallback.
...
unsafe fn get_hidden_definition_name(cx: &mut SafeJSContext, obj: *mut JSObject) -> Option<String> {
    ...
    if value.get().is_string() {
        Some(js_value_to_string(cx, value.get()))
    } else {
        None
    }
}
...
unsafe fn upgrade_node_by_id(cx: &mut SafeJSContext, node_id: usize, forced_definition: Option<&str>) {
    ...
    // Fire once on first upgrade even when connectivity detection is incomplete in this
    // engine; later upgrades still use connected-state transitions.
    invoke_connected_callback(cx, element_obj.get());
...
unsafe extern "C" fn custom_elements_define(raw_cx: *mut JSContext, argc: c_uint, vp: *mut JSVal) -> bool {
    ...
    if did_insert {
        ...
        // Define() should only upgrade matching elements currently in the document.
...
pub fn setup_custom_elements(runtime: &mut JsRuntime) -> Result<(), String> {
    runtime.do_with_jsapi(|cx, global| unsafe {
        let raw_cx = cx.raw_cx();
