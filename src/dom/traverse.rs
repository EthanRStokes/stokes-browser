...
impl<'a> TreeTraverser<'a> {
    pub fn new(dom: &'a Dom) -> Self {
        Self::new_with_root(dom, 0)
    }

    pub fn new_with_root(dom: &'a Dom, root: usize) -> Self {
        let mut stack = Vec::with_capacity(32);
        stack.push(root);
        TreeTraverser { dom, stack }
    }
}
...
impl<'a> AncestorTraverser<'a> {
    pub fn new(doc: &'a Dom, node_id: usize) -> Self {
        AncestorTraverser {
            doc,
            current: node_id,
        }
...
impl Dom {
    ...
    pub fn next_node(&self, start: &DomNode, mut filter: impl FnMut(&DomNode) -> bool) -> Option<usize> {
