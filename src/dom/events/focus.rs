use crate::dom::Dom;
use crate::events::{BlitzFocusEvent, DomEvent, DomEventData};

pub(crate) fn generate_focus_events(
    doc: &mut Dom,
    update_focus: &mut dyn FnMut(&mut Dom),
    dispatch_event: &mut dyn FnMut(DomEvent),
) {
    // Update focus, tracking which node was focussed before and after
    let old_focus = doc.get_focused_node_id();
    update_focus(doc);
    let new_focus = doc.get_focused_node_id();
