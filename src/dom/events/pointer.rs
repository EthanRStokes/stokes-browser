...
pub(crate) fn handle_pointerdown(
    ...
    doc.quick_clicks = if doc
        .last_mousedown_time
        .map(|t| t.elapsed() < Duration::from_millis(500))
        .unwrap_or(false)
        && (doc.mousedown_pos.x - x).abs() <= 2.0
        && (doc.mousedown_pos.y - y).abs() <= 2.0
    {
        doc.quick_clicks + 1
    } else {
        1
    ...
    doc.mousedown_pos = taffy::Point { x, y };
    ...
    match click_target {
        ...
        ClickTarget::TextInput { content_box_offset } => {
            ...
            let click_count = doc.quick_clicks;
            let node = &mut doc.nodes[actual_target];
            let el = node.data.element_mut().unwrap();
            if let SpecialElementData::TextInput(ref mut text_input_data) = el.special_data {
        ...
        while let Some(node_id) = maybe_node_id {
            ...
            match el.name.local {
                local_name!("input") if el.attr(local_name!("type")) == Some("checkbox") => {
                    let is_checked = Dom::toggle_checkbox(el);
                ...
                local_name!("input") if el.attr(local_name!("type")) == Some("radio") => {
                    ...
                    Dom::toggle_radio(doc, radio_set, node_id);
