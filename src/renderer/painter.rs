...
// painter
impl PaintScene for ScenePainter<'_> {
    fn reset(&mut self) {
...
mod sk_peniko {
    ...
    pub(in crate::renderer) fn shader_from_image_brush(
    ...
    pub(in crate::renderer) fn shader_from_gradient(
        gradient: &Gradient,
        brush_transform: Option<kurbo::Affine>,
    ) -> SkShader {
    ...
    pub(in crate::renderer) fn path_fill_type_from_fill(fill: Fill) -> SkPathFillType {
        match fill {
    ...
    pub(in crate::renderer) fn color4f_from_alpha_color(color: AlphaColor<Srgb>) -> SkColor4f {
        SkColor4f::new(
            color.components[0],
            color.components[1],
            color.components[2],
    ...
    pub(in crate::renderer) fn color4f_from_dynamic_color(color: DynamicColor) -> SkColor4f {
        SkColor4f::new(
            color.components[0],
            color.components[1],
            color.components[2],
    ...
    pub(in crate::renderer) fn gradient_shader_cs_from_cs_tag(
    ...
    pub(in crate::renderer) fn gradient_shader_hue_method_from_hue_direction(
    ...
    pub(in crate::renderer) fn tile_mode_from_extend(extend: Extend) -> SkTileMode {
        match extend {
    ...
    pub(in crate::renderer) fn blend_mode_from(blend_mode: BlendMode) -> SkBlendMode {
...
mod sk_kurbo {
    ...
    pub(in crate::renderer) fn rect_from(rect: Rect) -> SkRect {
        SkRect::new(
            rect.x0 as f32,
            rect.y0 as f32,
            rect.x1 as f32,
    ...
    pub(in crate::renderer) fn rrect_from(rrect: RoundedRect) -> SkRRect {
    ...
    pub(in crate::renderer) fn m44_from_affine(affine: Affine) -> SkM44 {
    ...
    pub(in crate::renderer) fn matrix_from_affine(affine: Affine) -> SkMatrix {
        let m = affine.as_coeffs();
        let scale_x = m[0] as f32;
        let shear_y = m[1] as f32;
        let shear_x = m[2] as f32;
    ...
    pub(in crate::renderer) fn pt_from(p: Point) -> SkPoint {
        SkPoint::new(p.x as f32, p.y as f32)
    }

    pub(in crate::renderer) fn path_from_shape(shape: &impl Shape) -> SkPath {
