...
impl Element<'_> {
    pub(super) fn draw_outset_box_shadow(&self, painter: &mut impl PaintScene) {
        ...
        let max_shadow_rect = box_shadow.iter().fold(Rect::ZERO, |prev, shadow| {
            let x = shadow.base.horizontal.px() as f64 * self.scale_factor;
            let y = shadow.base.vertical.px() as f64 * self.scale_factor;
            let blur = shadow.base.blur.px() as f64 * self.scale_factor;
            let spread = shadow.spread.px() as f64 * self.scale_factor;
        ...
        maybe_with_layer(
            ...
            |painter| {
                for shadow in box_shadow.iter().filter(|s| !s.inset).rev() {
                    let shadow_color = shadow.base.color.resolve_to_absolute(&current_color).as_color_color();

                    let alpha = shadow_color.components[3];
                    if alpha != 0.0 {
                        let transform = self.transform.then_translate(Vec2 {
                            x: shadow.base.horizontal.px() as f64 * self.scale_factor,
                            y: shadow.base.vertical.px() as f64 * self.scale_factor,
                        });
    ...
    pub(super) fn draw_inset_box_shadow(&self, scene: &mut impl PaintScene) {
        ...
        for shadow in box_shadow.iter().filter(|s| s.inset) {
            ...
            scene.draw_box_shadow(
                ...
                Color::WHITE,
                radius,
                shadow.base.blur.px() as f64 * self.scale_factor,
            );
